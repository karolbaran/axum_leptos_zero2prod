...
docker run \
  ...
  -p $DB_PORT_DOCKER:$DB_PORT \
  --name newsletter-axum-db \
  -d postgres:latest \
  postgres -N 1000

# Keep pinging the database until it is ready to accept commands
export PGPASSWORD=$DB_PASSWORD
until psql -h $DB_HOST -p $DB_PORT_DOCKER -U $DB_USER -d $DB_NAME -c '\q'; do
  >&2 echo "Postgres is unavailable - sleeping"
  sleep 1
done

>&2 echo "Postgres is up and running on port ${DB_PORT_DOCKER}!"
